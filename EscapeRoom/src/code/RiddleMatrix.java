...
public class RiddleMatrix {

	// verrückt spalten um angegebene distance
			...
			for (int i = 0; i < spalten; i++) {
				...
				if (listAsString.contains(chosenWordsArray[counter])) {
					if (listAsString.contains(searchedWord)) // Gelbe Zeile wird entfernt wenn Hauptwort gefunden wurde
						mainWordFound = true;
					if(gamemode == 1)
					pointSystem(counter);//points here moved from upper line to here, same down below
			...
			for (int i = 0; i < zeilen; i++) {
				...
				if (listAsString.contains(chosenWordsArray[counter])) {
					if (listAsString.contains(searchedWord))
						mainWordFound = true;
					if(gamemode == 1)
					pointSystem(counter);//					 points here
					positionOfHint = chosenWords.indexOf(chosenWordsArray[counter]);
					hintsForChosenWords.remove(positionOfHint);
...
//	private boolean isRunning = true;
	...
	protected int counterRow, counterCol, amountOfWordsInRiddle, minSizeOfMatrix, zeilen, spalten,
			positionOfSearchedWordChars, input, points = 0, maxPoints = 0, counterRowForList = 0, moveDistance, gamemode, neededPoints, outputTime;
	...
	void minSizeOfMatrix() {
		// stringListToCharList() hier einfügen
		Collections.shuffle(neededLetters); // mischt diese Buchstaben durch ( benötigt für später) bei einordnen in
											// Matrix
		minSizeOfMatrix = neededLetters.size() + searchedWord.length();
		spalten = searchedWord.length();
		...
		if (((secondLongestWord.length()) * (searchedWord.length()) >= minSizeOfMatrix) && secondLongestWord.length() >= chosenWords.size()) {
			zeilen = secondLongestWord.length();
			...
			zeilen = chosenWords.size() + minSizeOfMatrix / spalten;

		}
		outputTime = 10; 
		
		colorRow = (int) zeilen / 2;
	...
	void fillMatrixWithRandomValues() {
		...
		for (counterCol = 0; counterCol < spalten; counterCol++) { // Einzelne Buchstaben werden jeweils zufällig in
			// einer anderen Spalte plaziert (matrix)
			String charToString = "" + searchedWord.charAt(counterCol);
			positionOfSearchedWordChars = (int) (Math.random() * zeilen);
			array[positionOfSearchedWordChars][counterCol] = charToString;
			freeSpace[positionOfSearchedWordChars][counterCol] = false; // markiert freien Platz für restliche
			// Buchstaben [ohne main word]
		}
// füllt die Zellen die nicht von main Word belegt sind mit neededLetters raus
	...
	//wählt gameMode aus muss noch platziert werden
	void selectGamemode() {
		if(chosenWords.size()<3)
			gamemode = 0; //find all words
			
		if(chosenWords.size()>=3)
			gamemode = 1; //point game
